...
function IPCTable_toExcel(){
	...
	try{
	  ...
	  var mainHeader = document.getElementById("tableHeader");
...
function IPCItem_hlight(flag){
  if(this.isAttachingKeyword) return;
	...
	if(isMSIE)this.tr.style.backgroundColor = this.tr.style.backgroundColor; //Correct IE10 bug
	this.hlighted = flag;
}
...
this.oContentDiv = null;
this.oIframeShim = null;
this.isOpen=false;
this.coords = [0, 0, 0, 0];
this.isOver3D = false;
}
...
if(item!=null && (ipc_data.dplist[item.index].note || ipc_data.dplist[item.index].attachingKeyword))return;
	...
	if(item!=null){
		...
		if(links.length>0){
			for(var i=0; i<links.length; i++){
				tableSyntax += getOption(links[i].description, "openLink("+item.index+","+i+");", true);
			}
			tableSyntax += hrOption;
		}
	}
	...
	this.oContentDiv.className = "cmenuholder";
	this.oContentDiv.innerHTML = this.oContextHTML.innerHTML;
	var dstyle = this.oContentDiv.style;
	dstyle.display = "block";	
	this.oIframeShim = document.createElement("iframe");
	this.oIframeShim.frameBorder = 0;
	this.oIframeShim.scrolling = "no";
	this.oIframeShim.src = "about:blank";
	var ifrstyle = this.oIframeShim.style;

	dstyle.position = ifrstyle.position = 'absolute';
	dstyle.left = ifrstyle.left = x + 'px';
	dstyle.top = ifrstyle.top = y + 'px';
	dstyle.width = ifrstyle.width = wW + 'px';
	dstyle.height = ifrstyle.height = wH + 'px';
	dstyle.zIndex = 1000;
	ifrstyle.zIndex = dstyle.zIndex - 1;
	document.body.appendChild(this.oContentDiv);
	document.body.appendChild(this.oIframeShim);
...
function ContextMenu_close(){
	if(this.isOpen){
		this.isOpen = false;
		if(this.oContentDiv)this.oContentDiv.parentNode.removeChild(this.oContentDiv);
		if(this.oIframeShim)this.oIframeShim.parentNode.removeChild(this.oIframeShim);
		this.oContentDiv = null;
		this.oIframeShim = null;
...
//  Service Functions
//==============================================================================
function openLink(n, k){
	workTable.contextMenuClose();
	var link = ipc_data.dplist[n].links[k];
...
function initialize_api(data) {
	if(with3D){
	  ...
	  workTable = new IPCTable(document.getElementById("tableIPC"));
	  if(ipcApi.checkCortonaComponent()){
	  	  var verC3D = 7;
	  	  if(!isMSIE){
	  	  	try{verC3D = parseInt(cortona.Version, 10);}catch(err){}
	  	  }
		  ...
		  on_set_properties(data);
			...
			document.getElementById('cortonaContainer').innerHTML = '<table width="100%" height="100%"><tr><td align="center" valign="middle" style="white-space:normal;" class="errorMessage">'+installMessage+'</td></tr></table>';
...
function on_view_changed(sViewName) {
  ...
  workTable.update();
  document.getElementById("tbdContainer").style.visibility = "visible";
...
function on_doc_load(){
	...
	tableColsWidth = getCollumnsWidth();
	setCollumnsWidth(tableColsWidth);
		
	document.body.onresize = on_content_resize;
	document.getElementById('baseDiv').onmousedown = CloseContextMenu;
	on_content_resize();
...
function on_content_resize(){
	
	try{
		var cntrlWidth = document.body.offsetWidth-document.getElementById("tablePanel").offsetWidth+1;
		if(cntrlWidth<0)cntrlWidth=0;
		document.getElementById("viewersPanel").style.width=cntrlWidth+"px";
		var cntHeight = document.body.offsetHeight-document.getElementById("THeader").offsetHeight-document.getElementById("TFooter").offsetHeight;
		if(cntHeight<0)cntHeight=0;
		document.getElementById("tbdContainer").style.height=cntHeight+"px";
		var cHeight = document.body.offsetHeight-document.getElementById("cHeader").offsetHeight-document.getElementById("cFooter").offsetHeight;
		if(cHeight<0)cHeightt=0;
		document.getElementById("cContainer").style.height=cHeight+"px";
	}catch(err){}
	CloseContextMenu();
}
...
function updateTableHeader(){
	...
	document.getElementById("tableIPC").deleteTHead();
	var hdr = document.getElementById("tableIPC").createTHead();
	var visRows = document.getElementById("tableHeader").rows;
	for (var row=0; row<visRows.length; row++) {
		hdr.appendChild(visRows[row].cloneNode(true));
	 }	 
}
...
//	document.getElementById("chbxShowBoxes").onclick = function(){user_chkbxs = this.checked; hideCheckBoxesColumn(user_chkbxs);};
}

function attachUIEvents(){
	var uielements = [
	 ["btnCalloutsUndo","onclick",undoCallout]
	,["btnFitSelection","onclick",fit_3D_selection]
	,["btnShowSelection","onclick",show_3D_selection]
	,["btnHideSelection","onclick",hide_3D_selection]
	,["chkFullTable","onclick",function(){setFullTable(this.checked);}]
	,["chbxShowBoxes","onclick",function(){user_chkbxs = this.checked; hideCheckBoxesColumn(user_chkbxs);}]
	,["viewsSelector","onchange",function(){selectView(this.selectedIndex);}]
	,["ScrollSlider","onmousedown",function(){if(tSlider!=null)is_tSliderActive=true;}]
	,["ScrollSlider","onscroll",set_transparency_3D_selection]
//	,["btnCalloutsUndo","onscroll",undoCallout]
//	,["btnCalloutsUndo","onclick",undoCallout]
	];
	var uie, he;
	for(var i=0; i<uielements.length; i++){
		uie=uielements[i];
		he=document.getElementById(uie[0]);
		if(he)he[uie[1]]=uie[2];
	}
}
...
function presetHtml(data) {
	//activateUIElements();
	attachUIEvents();
	document.getElementById("tableIPC").onselectstart=function(){if(window.event.shiftKey || window.event.ctrlKey)return false;};
	...
	*/
	...
	if(viewer2D!=1)document.getElementById("btn_composite_print").style.display = "none";
	...
	if(with3D){
		...
		try{
			C_YELLOW = new Array(parseInt("0x"+clr3DSelectedColor.substr(1,2), 16)/255.0, parseInt("0x"+clr3DSelectedColor.substr(3,2), 16)/255.0, parseInt("0x"+clr3DSelectedColor.substr(5,2), 16)/255.0);
		}catch(e){}
...
function hideTable(flag){
	document.getElementById("tablePanel").style.display = (flag)? 'none': '';
	on_content_resize();
...
function TopLineControl(){
	this.blocked = true;
	this.buttonsNames = ["btn_3donly", "btn_skipanimation", "btn_ignoretransp", "btn_help", "btn_print_3d", "btn_print_table", "btn_reset", "btn_excel", "btn_callouts", "btn_2d", "btn_composite_print"];
	this.btns =new Array();
...
function on_3D_selection(selectedItems){
	...
	for(var i=0; i<selectedItems.length; i++){
		if(selectedItems[i].state>=0 && selectedItems[i].presentInSheet){
			selected3DItems.push(selectedItems[i]);
		}
	}
...
function enableCallouts(bFlag){
	if(with3D){
	  ...
	  calloutDraw = null;
		...
		document.getElementById("navigation_controls").style.display = (!isCalloutsMode)? '': 'none';
		document.getElementById("callouts_controls").style.display = (isCalloutsMode)? '': 'none';
...
function embed3DControl() {
	...
	if(with3D){
		...
		if(isMSIE){
			...
			var rn = (typeof(forceDirectX) != 'undefined' && !forceDirectX)? "" : '<param name="RendererName" value="DirectX Renderer"/>';
			document.body.style.cursor=resFolder+"hourglas.ani";
			document.getElementById('cortonaContainer').innerHTML = '<table id="loading_table" width="100%" height="100%"><tr><td align="center" valign="middle" class="loadingMessage">'+loadingText+'</td></tr></table><object style="display:none;" id="cortonaControl" width="100%" height="100%" type="application/x-oleobject" classid="CLSID:86A88967-7A20-11D2-8EDA-00600818EDB1" border="0"><param name="Src" value="'+ipc_data.vrmlfile+'"/><param name="ConsoleMode" value="0"/><param name="ContextMenu" value="False"/><param name="LoadDroppedScene" value="False"/><param name="HeadLight" value="True"/><param name="NavigationBar" value="0"/><param name="RendererHints" value="5168"/>'+rn+'<param name="RendererOptimization" value="0"/><param name="ShowLogo" value="False"/><param name="TravelSpeed" value="2"/><param name="viewpoint_transition_mode" value="0"/><param name="WaitForAllResources" value="True"/><param name="BackColor" value="&h'+BackColor+'"/><param name="PixelBufferAccess" value="True"/><param name="Skin" value="{1706B265-E103-4332-9871-7FEE6C37C699}"/></object> <div id="block_div" style="position:absolute; z-index:10; overflow:hidden; top:0; left:0; width:3000px; height:3000px; background-color:transparent;"><table width="100%" height="100%"><tr>&nbsp;<td></td></tr></table></div>';
			cortona = document.getElementById('cortonaControl');
			cortona.attachEvent('OnSceneLoaded', on_cortona_scene_loaded);
			cortona.attachEvent('OnSceneUnloaded', on_cortona_scene_unloaded);
			cortona.attachEvent('MouseDown', on_cortona_mouse_down);
			cortona.attachEvent('MouseUp', on_cortona_mouse_up);
			cortona.attachEvent('OnKeyDown', on_cortona_key_down);
			cortona.attachEvent('MouseMove', on_cortona_mouse_move);
			cortona.attachEvent('OnMouseOut', on_cortona_mouse_out);
			cortona.attachEvent('OnMouseEnter', on_cortona_mouse_enter);
		}else{
			var rn = (typeof(forceDirectX) != 'undefined' && !forceDirectX)? "" : 'renderername="DirectX Renderer"';
			document.getElementById('cortonaContainer').innerHTML = '<embed type="application/x-cortona" width="100%" height="100%" id="cortonaControl" src="'+ipc_data.vrmlfile+'" consolemode="0" contextmenu="false" vrml_background_color="'+clr3DBackgroundColor+'" cpuloading="80" loaddroppedscene="false" rendererhints="5168" '+rn+' rendereroptimization="0" headlight="true" navigationbar="0" pixelbufferaccess="true" skin="{1706B265-E103-4332-9871-7FEE6C37C699}" vrml_splashscreen="false" travel_speed="2" viewpoint_transition_mode="0" WaitForAllResourses="true" />';
			cortona = document.getElementById('cortonaControl');
		}		
	}
}
...
function embed2DControl() {
	if(with2D){
	  if(isMSIE){
		if(viewer2D==1){
				document.getElementById('Container2D').innerHTML = '<object id="cortona2DControl" classid="CLSID:1D4A81CB-25A3-4EB7-A75E-4F8A5386D8F8" width="100%" height="100%"></object>';
				Control2D = document.getElementById('cortona2DControl');
				Control2D.attachEvent('HotspotHit', cortona2DControl_HotspotHit);
				Control2D.attachEvent('HotspotEnter', cortona2DControl_HotspotEnter);
				Control2D.attachEvent('HotspotLeave', cortona2DControl_HotspotLeave);
				Control2D.attachEvent('HotspotMenu', cortona2DControl_HotspotMenu);
				Control2D.attachEvent('HotspotMenuSelect', cortona2DControl_HotspotMenuSelect);
		}else if(viewer2D==2){
				document.getElementById('Container2D').innerHTML = '<object id="cortona2DControl" classid="CLSID:865B2280-2B71-11D1-BC01-006097AC382A" width="100%" height="100%"></object>';
				Control2D = document.getElementById('cortona2DControl');
				Control2D.attachEvent('ObjectHit', on_Control2D_ObjectHit);
				Control2D.attachEvent('ContextMenuItemHit', on_Control2D_ContextMenuItemClick);
		}
	  }else{
...
function checkControl2D(){
		...
		if(isControl2DNotFound){
			if(viewer2D==1){
					document.getElementById('Container2D').innerHTML = '<table id="Container2D" width="100%" height="100%"><tr><td align="center" valign="middle" style="white-space:normal;" class="errorMessage">'+iCaptionRapid2DViewerNotFound+'</td></tr></table>';
...
function load_2D_graphics(filename){
	...
	if(viewer2D==2){
		...
		try{
			if(isCstmTlbr && parseInt(Control2D.GetVersion().substr(0,1), 10)>=6){
...
function get2DColor(sHtmlColor, defaultValue){
	...
	try{
		nColor = RGB(parseInt("0x"+sHtmlColor.substr(1,2), 16), parseInt("0x"+sHtmlColor.substr(3,2), 16), parseInt("0x"+sHtmlColor.substr(5,2), 16));
	}catch(e){}
	return nColor;
}
...
function on_Control2D_ObjectHit(nMouseBtn, strObject, sName, KeyState){
	...
	if(viewer2D==2){
		...
		if(contextItem!=null){
			...
			if(links.length>0){
				for(var i=0; i<links.length; i++){
					add2DContextItem(links[i].description, true, "openLink("+item.index+","+i+");");
				}
				add2DContextItem("---", false);
			}
		}
	...
	if(HotspotID!=""){
		...
		if(hsName!=""){
			...
			if(contextItem!=null){
				...
				if(links.length>0){
					for(var i=0; i<links.length; i++){
						cmenuObjs[MenuObj.AppendMenuItem(links[i].description, ((i==0)? 0x0020 : 0))]= "openLink("+item.index+","+i+");";
					}
				}
...
function hideCheckBoxesColumn(flag){
	...
	var imgvis = (flag)? "": "hidden";
	for (var i = 0, length = nodeList.length; i < length; i++){
		nodeList[i].style.visibility = imgvis;
	}	
	nodeList = document.querySelectorAll(".ccl");
	var brdrw = (flag)? "": "0";
	for (var i = 0, length = nodeList.length; i < length; i++){
		nodeList[i].style.borderRightWidth = brdrw;
	}
	var cws = [].concat(tableColsWidth);
	if(!flag)cws[1] = (isIE8)? 5 : 1;
	setCollumnsWidth(cws);

	on_content_resize();	
}
function _HideTC(tblName, flag){	
	var rows = document.getElementById(tblName).rows;
	//var dsp = (flag)? "": "none";
	var wd = (flag)? "": "0";
	for (var row=0; row<rows.length; row++) {
	  var cells = rows[row].cells;
	  //if(cells.length>1)cells[1].style.display = dsp;
	  if(cells.length>1)cells[1].style.width=wd;
	}
}
...
function set2D(flag){
	...
	hideCortona2D(!is2D);
	enableCallouts(false);
	document.getElementById("cFooter").style.display = (!is2D)? '': 'none';
	...
	document.getElementById("chbxShowBoxes").disabled = (is2D);
	hideCheckBoxesColumn(!is2D && user_chkbxs);
...
function getCollumnsWidth(){
	var headerTDs = getTableCollumnCells(document.getElementById('tableHeader'));
	var tableTDs = getTableCollumnCells(document.getElementById('tableIPC'));
	var wds = [];
...
function setCollumnsWidth(wds){
	var tbH = document.getElementById('tableHeader');
	var tbB = document.getElementById('tableIPC');
	tbH.tableLayout=tbB.tableLayout="";
	var headerTDs = getTableCollumns(tbH);
	var tableTDs = getTableCollumns(tbB);
	var sum = 0;
	var isCCL;
	for(var i=0; i<wds.length; i++){
		isCCL = false;
		if(i==1){
			var mtb = getCSSRule("screen", "col.col01");
			if(mtb){
				mtb.style.width = wds[i]+"px";
				isCCL = true;
			}
		}
		if(!isCCL){
	...
	var mtb = getCSSRule("screen", "table.ipc");
	
	if(mtb!=null){
		mtb.style.tableLayout="fixed";
		mtb.style.width=sum+"px";
	}
	var mftr = getCSSRule("screen", "#ipcfooter");
	if(mftr!=null){
		mftr.style.display="none";
	}
...
function getTableCollumnCells(table){
	var cols = new Array();
	var r = 0;
	var clmns = 0;
...
function getCSSRule(styleMedia, ruleName) {
	var sm = styleMedia.toLowerCase();
	var rn = ruleName.toLowerCase();
	var screenStyles = [];
	var rNames = [];
	for(var i = 0; i < document.styleSheets.length; i++){
		if((document.styleSheets[i].media.mediaText && document.styleSheets[i].media.mediaText.toLowerCase()==sm) || document.styleSheets[i].media==sm){
	...
	if(screenStyles.length>0){
		for(var z = 0; z < screenStyles.length; z++){
			...
			for(var i = 0; i < rules.length; i++){
				if(rules[i].selectorText.toLowerCase()==rn){
					return rules[i];
				}
			}
			}
...
function disableButton(bttnName, bFlag){
	...
	document.getElementById(bttnName).style.color = (bFlag)? "#777777": "";
}
...
function prepareGraphics(){
	var svgCode, ddiv;
	var hdrElement = document.getElementById("screenHeader").parentNode;
	var ftr = document.getElementById("footerContent").innerHTML;
	var sheetNameDiv = document.getElementById("metasheet");
	var syntax = "";
	var vs = [];
	var isUseSVG = !isMSIE;
	var nPages = 0;
	var chrmstl = "position:relative;";
	try{isUseSVG=document.implementation.hasFeature("http://www.w3.org/TR/SVG11/feature#BasicStructure", "1.1");}catch(err){}
	if(isUseSVG){
		var viewboxRe = /<svg[^>]*viewBox="\s*(\d+\.?\d*)\s+(\d+\.?\d*)\s+(\d+\.?\d*)\s+(\d+\.?\d*)\s*"[^>]*>/i;
		var currentSrc = Control2D.src;
		for(var i=rightViews.length-1; i>=0; i--){
			if(rightViews[i].cgmsrc && rightViews[i].cgmsrc!=""){
				Control2D.src = get_basepath()+rightViews[i].cgmsrc;
				svgCode = "";
				try{
					svgCode = Control2D.getSVGSource();
				}catch(err){}
				if(svgCode=="")svgCode = '<svg xmlns="http://www.w3.org/2000/svg"><g><title>Error</title><text y="45%" x="50%" text-anchor="middle" fill="#000000" font-size="20" font-family="Sans-serif">Image file transformation error</text><text y="50%" x="50%" text-anchor="middle" fill="#000000" font-size="16" font-family="Sans-serif">'+rightViews[i].cgmsrc+'</text><text y="53%" x="50%" text-anchor="middle" fill="#000000" font-size="7" font-family="Sans-serif">'+Control2D.src+'</text><text y="58%" x="50%" text-anchor="middle" fill="#000000" font-size="20" font-family="Sans-serif">'+C2D_version()+'</text></g></svg>';
				svgCode = svgCode.replace(/<\?xml[^>]+\?>/i,'');
				svgCode = svgCode.replace('width','oldW');
				svgCode = svgCode.replace('height','oldH');
				if(isFF){
					svgCode = svgCode.replace('<svg','<svg width="90%" heigth="90%"');
				}else{
					svgCode = svgCode.replace('<svg','<svg width="100%" heigth="100%"');
				}
				svgCode = svgCode.replace(/<\/desc>\s*<defs\/>\s*<g>\s*<image\s+width="[^"]+"\s+height="[^"]+"/,'</desc> <defs/> <g> <image width="100%" height="100%"');
				
				ddiv = document.createElement("div");
				sheetNameDiv[sInnerText]=rightViews[i].description;
				if(isChrome){
					chrmstl="position:absolute; top:"+(i*100)+"%; ";
					nPages++;
				}
				ddiv.innerHTML='<div class="extgr" style="width:98%; height:98%; page-break-after: always; '+chrmstl+'"><div class="grHeader">'+hdrElement.innerHTML+'</div><div style="height:85%; margin-top:3mm;">'+svgCode+'</div><div class="grFooter" style="position:absolute; bottom:0; width:100%;">'+ftr+'</div></div>';				
				TEMP_GRAPHICS.push(ddiv.firstChild);
				document.body.insertBefore(ddiv.firstChild, document.body.firstChild);
			}
		}
		if(isChrome){
			ddiv = document.createElement("div");
			ddiv.innerHTML='<style id="chromeCSS" type="text/css" media="print">#tableIPC{position:absolute; top:'+(nPages*100)+'%;}</style>';
			TEMP_GRAPHICS.push(ddiv.firstChild);
			document.body.insertBefore(ddiv.firstChild, document.body.firstChild);
		}
		Control2D.src = currentSrc;
		sheetNameDiv[sInnerText] = " ";
	}else if(isMSIE){
		for(var i=0; i<rightViews.length; i++){
			if(rightViews[i].cgmsrc && rightViews[i].cgmsrc!=""){
				vs.push(i);
				sheetNameDiv[sInnerText]=rightViews[i].description;
				syntax += '<table class="crvTable" style="width:100%;height:100%;border-collapse: collapse;"><tr><td class="revcol" rowspan="3">&nbsp;</td><td class="prnHdr">'+hdrElement.innerHTML+'</td></tr><tr><td class="tdCRV" align="center" valign="middle"><object class="RapidViewControl" style="width:190mm;height:200mm" id="RpdCntr'+i+'" classid="CLSID:1D4A81CB-25A3-4EB7-A75E-4F8A5386D8F8"></object></td></tr><tr><td class="prnFtr">'+ftr+'</td></tr></table>';
			}
		}
		sheetNameDiv[sInnerText] = " ";
		
		ddiv = document.createElement("div");
		ddiv.className="divCSC";
		ddiv.innerHTML=syntax;
		TEMP_GRAPHICS.push(ddiv);
		...
		if(vs.length>0 && typeof(document.getElementById("RpdCntr"+vs[0]).backColor)=='undefined'){
			...
			for(var i=0; i<vs.length; i++){
				try{
					document.getElementById("RpdCntr"+vs[i]).openFile(rightViews[vs[i]].cgmsrc, "");
				}catch(err){
					document.getElementById("RpdCntr"+vs[i]).outerHTML ='<div style="padding-top:50%;">Cortona2D Viewer ('+document.getElementById("Cntr2D_"+i).GetVersion()+') Error<br>Image file was not loaded:<br>'+rightViews[i].cgmsrc+'</div>';
				}
			}
		}
		}catch(err){}
...
function clearTempGraphics(){
	for(var i=0; i<TEMP_GRAPHICS.length; i++){
		TEMP_GRAPHICS[i].parentNode.removeChild(TEMP_GRAPHICS[i]);		
	}
	TEMP_GRAPHICS=[];
}
...
function fAfterPrint(){
	if(isSpPrn){
		clearTempGraphics();
		isSpPrn = false;
	  	if(specType!="GENERIC" && document.getElementById("metasheet"))document.getElementById("metasheet")[sInnerText] = view_description;
